...
export const Abilities: import("../../../sim/dex-abilities").ModdedAbilityDataTable =
	{
		...
		cudchew: {
			onEatItem(item, pokemon, source, effect) {
				if (
					item.isBerry &&
					(!effect || !["bugbite", "pluck"].includes(effect.id))
				) {
		...
		emergencyexit: {
			onSourceHit(target, source, move) {
				...
				this.add("-activate", source, "ability: Emergency Exit");
			},
			flags: {},
			name: "Emergency Exit",
			rating: 1,
		...
		iceface: {
			...
			onStart(pokemon) {
				if (
					this.field.isWeather(["hail", "snowscape"]) &&
			...
			onStart(pokemon) {
				...
				for (const target of pokemon.adjacentFoes()) {
					...
					if (target.volatiles["substitute"]) {
						...
						// Cambiamos 'atk' por 'accuracy'
						this.boost({ accuracy: -1 }, target, pokemon, null, true);
					}
				}
			},
			name: "Illuminate",
			rating: 1, // Le subo el rating porque ahora es útil en combate
			num: 35,
		},
		illusion: {
					...
					if (!possibleTarget.fainted) {
						...
						if (
							...
							!["Ogerpon", "Terapagos"].includes(
								possibleTarget.species.baseSpecies,
							)
						) {
			...
			onSwitchIn(pokemon) {
				...
				if (target) {
					pokemon.transformInto(
						target,
						this.dex.abilities.get("imposter"),
					);
				}
			},
			flags: { failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1 },
		...
		naturalcure: {
			onCheckShow(pokemon) {
				...
				if (!cureList.length || !noCureCount) {
					...
					this.add(
						"-message",
						`(${cureList.length} of ${pokemon.side.name}'s pokemon ${
		...
		neutralizinggas: {
			...
			onEnd(source) {
				...
				for (const pokemon of this.getAllActive()) {
					if (
						pokemon !== source &&
						pokemon.hasAbility("Neutralizing Gas")
					) {
						return;
					}
				}
		...
		parentalbond: {
			onPrepareHit(source, target, move) {
				if (
					...
					return;
				move.multihit = 2;
				move.multihitType = "parentalbond_z_mod" as any;
			},
			...
			onBasePower(basePower, pokemon, target, move) {
				if ((move.multihitType as string) === 'parentalbond_z_mod' && move.hit === 2) {
					return this.chainModify(0.5);
				}
			},
			flags: {},
			...
			onResidual(pokemon) {
				...
				pokemon.canMegaEvo =
					pokemon.canMegaEvo === false
						? false
						: this.actions.canMegaEvo(pokemon);
		...
		supremeoverlord: {
			...
			onBasePower(basePower, attacker, defender, move) {
				if (this.effectState.fallen) {
					...
					this.debug(
						`Supreme Overlord boost: ${
							powMod[this.effectState.fallen]
						}/4096`,
					);
		...
		wanderingspirit: {
			onDamagingHit(damage, target, source, move) {
				...
				if (this.checkMoveMakesContact(move, source, target)) {
					...
					const sourceAbility = source.setAbility(
						"wanderingspirit",
						target,
					);
		...
		overload: {
			...
			num: -4,
			rating: 2,
			desc: "Powers up Electric-type moves by 50% when the Pokémon's HP is low (1/3 or less).",
			shortDesc: "At 1/3 or less HP, Electric-type moves have 1.5x power.",
		},
		sagepower: {
			//Poder Sabio
			onStart(pokemon) {
			...
			rating: 4.5,
			num: -5,
			desc: "The Pokémon's Special Attack is increased by 50%, but it can only use the first move it selects.",
			shortDesc: "1.5x Sp. Atk, but can only use the first selected move.",
		},
		coleopterous: {
			//Coleóptero
			onModifyTypePriority: -1,
			...
			shortDesc: "Normal moves become Bug-type.",
		},
		spectrate: {
			//Piel Tétrica
			onModifyTypePriority: -1,
			...
			rating: 4,
			num: -7,
			desc: "Normal-type moves become Ghost-type moves and their power is boosted.",
			shortDesc: "Normal moves become Ghost-type and have 1.2x power.",
		},
		dread: {
			onStart(pokemon) {
				let activated = false;
			...
			rating: 3.5,
			num: -8,
			desc: "Upon entering battle, the Pokémon lowers the Special Attack of opposing Pokémon.",
			shortDesc: "On switch-in, lowers the foes' Sp. Atk by 1 stage.",
		},
		icehead: {
			//Iceberg
			onDamage(damage, target, source, effect) {
			...
			rating: 3,
			num: -9,
			desc: "The Pokémon does not take recoil damage.",
			shortDesc: "Prevents recoil damage.",
		},
		flammable: {
			// Inflamable
			onModifyAtkPriority: 5,
			...
			rating: 4.5,
			num: -10,
			desc: "Increases the damage of Fire-type moves.",
			shortDesc: "Power of Fire-type moves is increased.",
		},
		albinism: {
			// Albinismo
			onModifyAtkPriority: 5,
			...
			rating: 4.5,
			num: -11,
			desc: "Increases the damage of Ice-type moves.",
			shortDesc: "Power of Ice-type moves is increased.",
		},
		botanize: {
			// Piel Herbácea
			onModifyTypePriority: -1,
			...
			rating: 4.5,
			num: -12,
			desc: "Normal-type moves become Grass-type moves and their power is boosted.",
			shortDesc: "Normal moves become Grass-type and have 1.2x power.",
		},
		blooming: {
			// Floración
			onModifyAtkPriority: 5,
			...
			rating: 4.5,
			num: -13,
			desc: "Increases the damage of Grass-type moves.",
			shortDesc: "Power of Grass-type moves is increased.",
		},
		royalty: {
			// Realeza
			onModifySTAB(stab, source, target, move) {
				if (stab >= 1.5) {
					// Si ya tiene STAB (1.5), lo sube a 2 (como Adaptable)
					return 2;
				}
				// Si no tiene STAB (1), le da el bonus básico de 1.5
				return 1.5;
			},
			name: "Royalty",
			rating: 6,
			num: -14,
			desc: "The Pokémon receives a 50% damage bonus when attacking with any type.",
			shortDesc: "Deals 1.5x damage with moves of any type.",
		},
		onslaught: {
			// Acometida
			onStart(pokemon) {
				// Marcamos que es el primer turno del Pokémon en el campo
				pokemon.addVolatile("acometida");
			},
			onEnd(pokemon) {
				// Limpiamos el estado si el Pokémon sale del campo
				pokemon.removeVolatile("acometida");
			},
			condition: {
				duration: 1, // Dura solo el primer turno
				onStart(pokemon) {
					this.add("-ability", pokemon, "Acometida");
					this.add(
						"-message",
						`${pokemon.name} está listo para la acometida!`,
					);
				},
			...
			rating: 4.5,
			num: -15,
			desc: "On the first turn the Pokémon is in battle, its Speed is increased by 50% and its Attack by 20%.",
			shortDesc: "On first turn: 1.5x Speed and 1.2x Attack.",
		},
		chillypower: {
			// Poder Gélido
			onModifyAtkPriority: 5,
			onModifyAtk(atk, attacker, defender, move) {
				if (
					move.type === "Ice" &&
					["snow", "hail"].includes(attacker.effectiveWeather())
				) {
					this.debug("Poder Gélido boost");
					return this.chainModify(1.5);
				}
			},
			onModifySpAPriority: 5,
			...
			rating: 4.5,
			num: -16,
			desc: "Increases the Pokémon's Ice-type move power and Speed by 50% during snow.",
			shortDesc: "Ice power and Speed are 1.5x during Snow.",
		},
		sylvan: {
			// Silvano
			onBasePowerPriority: 23,
			...
			rating: 4.5,
			num: -17,
			desc: "Increases the damage of any move by 30% while Grassy Terrain is active.",
			shortDesc: "Moves deal 1.3x damage under Grassy Terrain.",
		},
		brawler: {
			// Camorrista
			onBasePowerPriority: 23,
			onBasePower(basePower, attacker, defender, move) {
				const moveFlags = move.flags as any;
				if (moveFlags && moveFlags["kick"]) {
		...
		tinkle: {
			...
			onStart(pokemon) {
				...
				this.add(
					"-message",
					`¡El tintineo de ${pokemon.name} sanó los estados del equipo!`,
				);
			},
			onModifyTypePriority: -1,
			onModifyType(move, pokemon) {
				if (
					...
					move.type = "Psychic";
					(move as any).tintineoBoosted = true;
				}
			},
			onBasePowerPriority: 23,
			onBasePower(basePower, pokemon, target, move) {
				if ((move as any).tintineoBoosted)
					return this.chainModify([4915, 4096]);
			},
			name: "Tinkle",
			rating: 4.5,
			num: -19,
			desc: "Cures the status conditions of the Pokémon's team upon entering battle. Additionally, sound-based moves become Psychic-type and their power is increased by 20%.",
			shortDesc:
				"Cures team status on switch-in. Sound moves become Psychic-type with 1.2x power.",
		},
		sturdyroot: {
			//Raíz Robusta
			onTryHealPriority: 1,
			...
			rating: 4.5,
			num: -20,
			desc: "The Pokémon recovers more HP from draining moves.",
			shortDesc: "HP recovered from draining moves is increased.",
		},
		burntouch: {
			// Toque Ardiente
			onSourceDamagingHit(damage, target, source, move) {
				// Bloqueado por Polvo Escudo y Capa Furtiva (igual que Poison Touch)
				if (
					target.hasAbility("shielddust") ||
					target.hasItem("covertcloak")
				)
			...
			rating: 4.5,
			num: -21,
			desc: "The Pokémon may burn the target on contact a 30% of the time.",
			shortDesc: "30% chance to burn the target on contact.",
		},
		vilealchemy: {
			onResidualOrder: 28,
			onResidualSubOrder: 2,
			onResidual(pokemon) {
				...
				for (const target of pokemon.foes()) {
					...
					if (target.status === "tox") {
						const stage = target.statusState.stage || 1;
						const toxicDamage = (stage * target.baseMaxhp) / 16;
						this.damage(toxicDamage, target, pokemon);
					}
				}
			},
			flags: {},
		...
		counterguard: {
			...
			onTryHit(target, source, move) {
				if (target !== source && move.type === "Flying") {
					this.add("-immune", target, "[from] ability: Counterguard");
					this.damage(source.baseMaxhp / 8, source, target);
					return null; // Esto cancela el movimiento por completo
				}
			},
			flags: { breakable: 1 },
			name: "Counterguard",
		...
		flamewaker: {
			...
			onTryHit(target, source, move) {
				if (target !== source && move.type === "Fire") {
					if (!this.boost({ spa: 1 })) {
						this.add("-immune", target, "[from] ability: Wakeflame");
					}
					return null;
				}
			},
		...
		illusoryform: {
			onUpdate(pokemon) {
				if (pokemon.hp > pokemon.maxhp * 0.75) {
					// Si tiene la vida alta y no está al máximo de evasión (+6), subimos
					if (pokemon.boosts.evasion < 6) {
						this.boost({ evasion: 2 }, pokemon, pokemon);
					}
				} else {
					// Si la vida baja del 75%, reseteamos la evasión
					if (pokemon.boosts.evasion > 0) {
						this.boost({ evasion: 0 }, pokemon, pokemon);
						this.add(
							"-description",
							pokemon,
		...
		porkodor: {
			onDamagingHit(damage, target, source, move) {
				if (move.flags["contact"]) {
					this.add("-ability", target, "Porkodor");
					this.boost({ evasion: -2 }, source, target);
				}
			},
			flags: {},
			name: "Pork Odor",
		...
		piercing: {
			onModifyMove(move) {
				// Comprobamos si el movimiento es de corte/punzante y si tiene ratio de crítico alto
				if (move.flags["slicing"] && move.critRatio && move.critRatio > 1) {
					if (!move.secondaries) move.secondaries = [];
					move.secondaries.push({
						chance: 30,
						volatileStatus: "flinch",
		...
		oddpresence: {
			onStart(pokemon) {
				if (this.field.getPseudoWeather("trickroom")) {
					// Si ya hay Espacio Raro, no hacemos nada o enviamos un mensaje de depuración
					return;
				}
				this.add("-ability", pokemon, "Odd Presence");
		...
		magictouch: {
			onModifyMove(move) {
				if (!move.secondaries) move.secondaries = [];
				this.debug("Magic Touch adding confusion chance");
				move.secondaries.push({
					chance: 20,
