...
export const Conditions: import("../../../sim/dex-conditions").ConditionDataTable =
	{
		/*
		Estado Hemorragia
		Provoca que sea mas probable recibir un critico +1 de indice de critico
		Si un pokemon tiene un 4% de normal (indice 0 de critico) si sufre hemorragia tendra un 12% (+1 de indice de critico)
		Movimientos con alto indice de critico (+1 de indice 12%) pasarían a un 50% (+2 de indice de critico)
		Si ademas tuviera afortunado seria un100% de critico con la hemorragia y el movimiento de alto indice de critico (en total +3 de indice de critico)
		*/
		bld: {
			name: "bld",
			effectType: "Status",
			onStart(target) {
				this.add("-start", target, "condition: Bleeding");
			},
			// El efecto se aplica en el atacante cuando calcula su ratio contra el poseedor del estado
			onModifyCritRatio(critRatio, source, target) {
				return critRatio + 1;
			},
			onResidualOrder: 9,
			onResidual(pokemon) {
				this.add("-activate", pokemon, "condition: Bleeding");
				// Si quieres que la hemorragia también quite algo de vida por turno, se añadiría aquí.
			},
		},
		/*
		Estado Caduco
		Siempre que tenga menos de 50% de vida un pokemon con este estado recibira el doble de daño
		Baya enigma cura el estado caduco
		*/
		dcy: {
			name: "dcy",
			effectType: "Status",
			onStart(target) {
				this.add("-start", target, "condition: Decay");
			},
			// Siempre que el poseedor tenga menos del 50% de vida, recibe el doble de daño
			onDamage(damage, target, source, effect) {
				if (
					...
					this.debug("Decay boost");
					return damage * 2;
				}
			},
			onEnd(target) {
				this.add("-end", target, "condition: DEcay");
			},
		},
		brn: {
			name: "brn",
		...
		frz: {
			...
			onStart(target, source, sourceEffect) {
				...
				// Mantenemos la mecánica de Shaymin-Cielo por si acaso
				if (
			...
			// IMPORTANTE: Ya no hay onBeforeMove, el Pokémon siempre ataca.

			// Reducción del Ataque Especial un 50%
			onModifySpAPriority: 1,
			onModifySpA(spa, pokemon) {
				return this.chainModify(0.5);
			},
			// Daño residual 1/16 al final de cada turno
			onResidualOrder: 9,
			onResidual(pokemon) {
				this.damage(pokemon.baseMaxhp / 16);
			},
			// Solo permitimos que se descongelen si ELLOS usan movimientos específicos de calor
			// Esto es coherente con que el usuario "entre en calor" para quitarse el frío.
		...
		partiallytrapped: {
			...
			onStart(pokemon, source) {
				...
				this.effectState.boundDivisor = source.hasItem("bindingband")
					? 6
					: 8;
			},
			onResidualOrder: 13,
